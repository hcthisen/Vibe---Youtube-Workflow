...
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS llm_model VARCHAR(50) DEFAULT 'gpt-4';
...
COMMENT ON COLUMN public.profiles.llm_model IS 'OpenAI model to use for retake analysis. Options: gpt-4, gpt-4-turbo, gpt-4o. Default: gpt-4';
...
ALTER TABLE public.profiles ADD CONSTRAINT llm_model_valid 
    CHECK (llm_model IN ('gpt-4', 'gpt-4-turbo', 'gpt-4o'));
